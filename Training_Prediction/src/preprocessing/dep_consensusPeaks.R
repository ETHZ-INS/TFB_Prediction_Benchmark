.mergeConservative <- function(bedFilePaths, minOverlap=1){
  ...
  # check if is replicated
  
  bed2 <- copy(bed)
  setkey(bed2, chr, start, end)
  bed <- foverlaps(bed, bed2, 
                   by.x=c("chr", "start", "end"),
                   by.y=c("chr", "start", "end"), 
                   minoverlap=minOverlap, 
                   nomatch=NULL)
  bed[,is_rep:=fifelse(peak_id!=i.peak_id & experiment!=i.experiment, TRUE, FALSE)]
  
  bed <- bed[,.(chr=data.table::first(chr),
                start=data.table::first(start),
                end=data.table::first(end),
                dist_center=data.table::first(dist_center),
                is_rep=sum(is_rep)>0,
                qValue=data.table::first(qValue)), by=.(peak_id)]
  
  bed <- makeGRangesFromDataFrame(as.data.frame(bed), keep.extra.columns=TRUE)
  bed <- unique(bed)
  
  # map back the max qvalue ov with the reduced bed
...
consensusPeaks <- function(consBedFilePaths, 
  ...
  if(length(optBedFilePaths)>0){
    optMergedPeaks <- .mergeOptimal(optBedFilePaths, minOverlap=1, 
                                    mergeRounds=min(length(unique(names(optBedFilePaths))), 2)) # length(unique(names(optBedFilePaths))) - 1

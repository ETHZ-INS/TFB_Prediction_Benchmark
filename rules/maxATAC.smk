...
def maxatac_targets(ckpt):
    ...
    # accessions = get_accession_chip_coverage(None)
    # 
    # proc_contexts= [
    #     f"data/maxATAC/03_normalized_atac/{context}_minmax.bw"
    #     for context in contexts_list()
    # ]
    # 
    # proc_combs = [
    #     f"data/maxATAC/03_normalized_chip/{comb}_minmax.bw"
    #     for comb in combsTrain
    # ]
    # 
    # tfs, contexts = zip(*(s.rsplit("_", 1) for s in combs)) if combs else ([], [])
    # proc_tfs = [
    #    f"models/maxATAC/{HOLDOUT}/{EPOCHS}/all/{tf}/best_epoch.txt"
    #    for tf in tfs
    # ]
...
rule maxATAC_download_annotation_data:
  ...
  shell:
    """
      maxatac data --output $(pwd)/{params.data_dir} >> {output.downloaded_data} &> {log}
    """
...
rule maxATAC_prepare_metadata:
  ...
  shell:
    """
      ...
      Rscript -e 'rmarkdown::render("src/maxATAC/01_metaData_maxATAC.Rmd", 
                                      "html_document", 
...
# TODO: Atm it predicts on all contexts (train & test) for one TF!! => see target builder

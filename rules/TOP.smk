...
rule TOP_normalize_count_chip_reads:
  input:
    # need an input function !! (as for maxATAC) definining all the accessions
    #chip_bam="data/common/02_cleaned_chip_coverage/{accession}.bam",
    cand_sites="data/TOP/motif_matches/candidate_sites_{tf}.tsv",
    bam_files=group_accession_chip_context_inputs
  output:
    norm_count_matrix="data/TOP/chip_read_counts/count_matrix_{tf}_{cellularContext}.rds"
  params:
    chrom_sizes_path="data/annotation/chr.sizes"
  benchmark: "benchmarks/TOP/prep_normalize_count_chip_reads_{tf}_{cellularContext}.tsv"
  log: "logs/TOP/normalize_count_chip_reads_{tf}_{cellularContext}.log"
  threads: 4
  #conda: "../envs/top.yaml"
  script: "../src/TOP_normalize_chip_coverage.R"
